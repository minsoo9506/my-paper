...
def _normalize_data(PATH: str, data_name: str, train_end_idx: int):
...
def _make_y_label(data_len: int, data_name: str):
    """make label of data
...
def split_train_valid_test(PATH: str, data_name: str, train_ratio: float = 0.8):
    ...
    normalized_data = _normalize_data(PATH, data_name, train_end_idx)

    data_len = len(normalized_data)
    y_label = _make_y_label(data_len, data_name)
